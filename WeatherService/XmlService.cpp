...
#include "C:\Users\Ilya\source\repos\WeatherService\pugixml-1.13\pugixml-1.13\src\pugixml.hpp"
...
Weather XmlService::getWeather(std::string s)
{
    ...
    double temperature = node.child("temperature").
        attribute("value").
        as_double(); // 5.69
    std::string weather = node.child("city").child("weather").
        attribute("value").
        as_string(); // дождь
    double windSpeed = node.child("city").child("wind").
        child("speed").
        attribute("value").
        as_double(); // 4.27
    int clouds = node.child("city").child("clouds").
        attribute("value").
        as_int(); // 100
